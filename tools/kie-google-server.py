...
IMPORTANT — Endpoint architecture:
  This server uses the DEDICATED Veo 3.1 endpoint:
    create:  POST https://api.kie.ai/api/v1/veo/generate
    poll:    GET  https://api.kie.ai/api/v1/veo/record-info?taskId=...
  Only Veo models below are compatible.

Available models for THIS server (Veo 3.1 endpoint):
  veo3        Google Veo 3.1 — highest quality, most credits
  veo3_fast   Default. Good balance of quality and speed
  veo3_lite   Fastest/cheapest; supports REFERENCE_2_VIDEO (up to 3 ref images)
...
UA = "Mozilla/5.0 (compatible; kie-google-mcp/1.0)"
SERVER_NAME = "kie-google"
...
def log(*a):
    print("[kie-google]", *a, file=sys.stderr, flush=True)
...
def generate_google_video(args):
    prompt = args["prompt"]
...
def handle(req):
    ...
    if method == "initialize":
        ...
        if name != "generate_google_video":
            send({"jsonrpc": "2.0", "id": rid, "result": {
                "content": [{"type": "text", "text": f"Unknown tool: {name}"}], "isError": True}})
            return
        try:
            text = generate_google_video(args)
